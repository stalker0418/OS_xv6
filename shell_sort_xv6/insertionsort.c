  ...
  while((n = read(fd, buf, sizeof(buf))) > 0) {
    ...
    while(k<n){
  	...
  	while(buf[k] != '\n'){
  		...
  		//printf(1,"%c",sentence[word_count-1]);
  	}
  	line_Array[z] = (char*)malloc((100)*sizeof(char)); 
     ...
     line_Array[no_of_lines] = " ";
     //sorting Algorithm using insertion sort
     int i=1,j=0;
     while(i<no_of_lines){
     	j=i-1;
     	char key[512];
     	strcpy(key,line_Array[i]);
     	if(strcmp(argv[1],"-n") == 0){
	  	while(j>=0 && atoi(line_Array[j])>atoi(key)){
		     	strcpy(line_Array[j+1],line_Array[j]);
		     	j--;
		}
	  }
	else{
		while(j>=0 && strcmp(line_Array[j],key)>0){
     			strcpy(line_Array[j+1],line_Array[j]);
     			j--;
     		}
	}
     	
     	strcpy(line_Array[j+1],key);
     	i++;
     }
